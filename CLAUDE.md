...
> 3. Update the interview note at `B:\Obsidian Vault\Interviews\Portfolio.md` (see Interview Note Template in the global CLAUDE.md) if this request changed the tech stack, architecture, features, challenges solved, or anything else worth mentioning in an interview. Skip only if the change is truly interview-irrelevant (e.g. typo fix, formatting).
> 4. Append an entry to `B:\Obsidian Vault\log.md` using format: `## [YYYY-MM-DD] project | Portfolio — description`
...
`B:\Obsidian Vault\Interviews\Portfolio.md`

---

## Recent Changes

- **2026-09-08** — Swapped the downloadable resume (`public/resume.pdf`) for `HHR_Sept2026.pdf` from `B:\Job Applier\resume\` (MD5-verified match, 182249 bytes). The working tree had a stale, uncommitted, different resume.pdf sitting there from an earlier session (never matched what was committed either) — replaced it outright rather than layering another swap on top. Branch `chore/update-resume-sept2026`, merged to `main`, pushed — Vercel auto-deploys. Left the pre-existing unrelated uncommitted deletion of `public/Hanan_Hussain_Resume_May2026.pdf` untouched, not part of this request.
- **2026-09-08** — Added two Motion Fitness client projects to `projects.ts` (project-13 Motion Fitness AI Caller, project-14 Motion Fitness Accounting Dashboard), inserted right after LOBEngine. AI Caller (`github`/`live` both `"#"` — self-hosted client-owned code, not a public repo, no public demo of live phone infra) covers the self-hosted AI phone receptionist (LiveKit Agents, Deepgram, Claude Haiku, Cartesia, Telnyx, deployed on AWS Lightsail). Accounting Dashboard (`github` → real repo `HananProjects/MFAccountingSoftware`) covers the separate bank-statement-parsing/bookkeeping tool for the same client. Did not add a third entry for Motion Fitness Task Board since it was superseded and merged into the AI Caller's dashboard in an earlier session — no standalone deliverable left to show. Branch `feature/motion-fitness-portfolio-entries`, `tsc --noEmit` clean, left uncommitted for review before merging to `main` (left two pre-existing unrelated uncommitted resume file changes in the working tree untouched, not part of this request).
- **2026-08-12** — Fixed outdated "student" status in the hero bio and page metadata (branch `fix/graduate-status-text`, merged to `main`, pushed). `src/app/layout.tsx`'s title/OG/Twitter `description` fields and `src/app/page.tsx`'s hero prose both still said "Computer Engineering student at USask... graduating June 2026" — Hanan graduated in June 2026 and has been working full-time since (Keytrus, Motion Fitness, Autoholic), so this was stale. Changed to "Computer Engineering graduate (USask)" / "graduate of the University of Saskatchewan (June 2026)". Caught mid-conversation while updating Motion Fitness's resume bullets — also prompted adding step 5 to the Universal Mandatory Update Protocol in the B:\ master `CLAUDE.md`: any resume-worthy project/milestone or status change (graduation, new role, etc.) should trigger a portfolio check. Also fixed real local git corruption found in the process: Google Drive sync had duplicated several `.git/objects/*` paths with `" (1)"` suffixes, which left `refs/heads/main`'s HEAD commit object unreadable (`bad object HEAD`, `git status`/`git log` failing entirely) — `git fetch origin` re-pulled the missing object from GitHub cleanly, then the redundant `" (1)"` duplicate files/dirs (confirmed byte-identical to their correctly-named counterparts) were deleted; `git fsck --full` clean afterward. Note for next session: this working tree's line endings are CRLF while git history is LF (no `.gitattributes`, no `core.autocrlf` set) — every file in the repo shows as modified in `git status` as a result. Only normalized the two files touched here (`perl -pi -e 's/\r\n/\n/g'`) to keep the diff to the actual content change (7 lines) rather than a whole-file rewrite; the rest of the pre-existing CRLF drift (and the several other pre-existing uncommitted files — `ui-ux-pro-max` data/scripts, `public/resume.pdf`, deleted `Hanan_Hussain_Resume_May2026.pdf`, `icon.svg`, `not-found.tsx`, `projects/[slug]/page.tsx`, `FileTree.tsx`, `projects.ts`) was left untouched, not part of this request.
...
- 15 total projects in `projects.ts`, displayed in array order (newest first): LOBEngine, Motion Fitness AI Caller, Motion Fitness Accounting Dashboard, Keytrus, FraudGuard, NaniStack, Autoholic Invoicing, Autoholic Website, AI Code Review Pipeline, Horus, ASL Translator, KawaKraft, Microprocessor, Booth Multiplier, Travel App. (Keyhold was renamed to Keytrus after the note below was originally written — `github` → `HananProjects/Keytrus`, `live` → `https://key-trus.vercel.app`; see git log, not this file, for the rename commits.)
- Motion Fitness AI Caller (project-13, added 2026-09-08): `github`/`live` both `"#"` deliberately — self-hosted, client-owned production code answering a real phone line, not a public repo or a link that's safe to expose. Motion Fitness Accounting Dashboard (project-14, added 2026-09-08): `github` → real repo `https://github.com/HananProjects/MFAccountingSoftware`; `live` stays `"#"` (local-only Flask/SQLite tool, nothing deployed). Branch `feature/motion-fitness-portfolio-entries` not yet merged to `main` — pending Hanan's review.
